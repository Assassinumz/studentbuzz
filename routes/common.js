...
router.get('/get-all-unis', async (req, res) => {
...
router.get('/findType', async (req, res) => {
  ...
  console.log(uid, "uid")
  const student = await Student.findOne({ uuid: uid })
  if (student) {
    res.json({ student: true, name: student.name })
    return
  }
  const faculty = await Faculty.findOne({ uuid: uid })
  if (faculty) {
    res.json({ faculty: true, name: faculty.name })
    return
  }
  const uni = await University.findOne({ uuid: uid })
  if (uni) {
    res.json({ uni: true, name: uni.name })
    return
  }
...
router.post('/login-uni', async (req, res) => {
  ...
  try {
    ...
    const uniObj = await University.findOne({ uuid: user.uid })
    return res.json({ uid: user.uid, data: uniObj })
  } catch (e) {
...
router.post('/login-fac', async (req, res) => {
  try {
    let data = req.body;
    console.log(data)
    let user = await LoginFirebaseUser(data.email, data.password)
    let uid_user = user.uid
    const faciii = await Faculty.findOne({ uuid: uid_user })

    return res.json({ uid: user.uid, data: faciii })
  }
  catch (e) {
    res.status(400).json({ error: e.message })
  }
